...
/usr/bin/mysql -uroot -p$MYSQL_ROOT_PASSWORD -e "CREATE USER 'zenoss'@'zenoss4-core' IDENTIFIED BY 'zenoss';"
/usr/bin/mysql -uroot -p$MYSQL_ROOT_PASSWORD -e "GRANT REPLICATION SLAVE ON *.* TO 'zenoss'@'zenoss4-core' IDENTIFIED BY PASSWORD '*3715D7F2B0C1D26D72357829DF94B81731174B8C';"
/usr/bin/mysql -uroot -p$MYSQL_ROOT_PASSWORD -e "GRANT ALL PRIVILEGES ON zodb.* TO 'zenoss'@'zenoss4-core';"
/usr/bin/mysql -uroot -p$MYSQL_ROOT_PASSWORD -e "GRANT ALL PRIVILEGES ON zenoss_zep.* TO 'zenoss'@'zenoss4-core';"
/usr/bin/mysql -uroot -p$MYSQL_ROOT_PASSWORD -e "GRANT ALL PRIVILEGES ON zodb_session.* TO 'zenoss'@'zenoss4-core';"
/usr/bin/mysql -uroot -p$MYSQL_ROOT_PASSWORD -e "GRANT SELECT ON mysql.proc TO 'zenoss'@'zenoss4-core';"
